...
`theaamghar-admin`'s job (`E:\Claude\theaamghar-admin`, Supabase project
...
*there*, not here. See `theaamghar-admin/CLAUDE.md`'s "This is now the real
...
storefronts belong on `theaamghar-web`'s already-mature engine, not
...
now pointing at the vendor's real `theaamghar-web`/`theaamghar-admin`
...
`vendor_admins` rows that then get provisioned into `theaamghar-admin`
...
reconciliation across vendors — distinct from `theaamghar-admin`'s
...
`theaamghar-web` side instead), real Resend email wiring.

### Domain plan

`superadmin.nashemann.store` → this app. Every vendor's real admin
subdomain (`admin.{slug}.nashemann.store`) → `theaamghar-admin`'s Vercel
