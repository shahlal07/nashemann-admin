...
function safeHandler(handler: (req: Request) => Promise<Response>) {
  return async (request: Request): Promise<Response> => {
    try {
      return await handler(request);
    } catch (error) {
      if (error instanceof Response) return error;
      const message = error instanceof Error ? error.message : "Vendor admin operation failed.";
      console.error("[Platform Admins API] Unhandled error:", error);
      return NextResponse.json({ error: message, code: "PLATFORM_ADMIN_ERROR" }, { status: 500 });
    }
  };
}
