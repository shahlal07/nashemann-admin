...
export function VendorDetailClient({ vendor: initialVendor, initialAdmins, categorySchema, health, churnRisk, isFinanceStaff }: { vendor: VendorRow; initialAdmins: VendorAdminRow[]; categorySchema: CategorySchemaRow; health: VendorHealth; churnRisk: ChurnRisk; isFinanceStaff: boolean; }) {
  ...
  async function saveAdminCredentials(e: React.FormEvent) {
    ...
    try {
      const updated = await updateVendorAdminAction(vendor.id, vendor.name, current.id, { name: editAdminName, email: editAdminEmail, password: editAdminPassword || undefined, previousEmail: current.email }, vendor.subdomain);
  ...
  async function removeAdmin(id: string) { const target = admins.find(a => a.id === id); setBusy(true); setError(null); try { await removeVendorAdminAction(vendor.id, vendor.name, id, target ? `${target.name} (${target.email})` : id, target?.email); setAdmins(prev => prev.filter(a => a.id !== id)); } catch (err) { setError(err instanceof Error ? err.message : "Couldn't remove admin"); } finally { setBusy(false); } }
  async function revokeSessions(admin: VendorAdminRow) { setBusy(true); setError(null); try { await revokeVendorAdminSessionsAction(vendor.id, vendor.name, admin.id, admin.name, admin.email); } catch (err) { setError(err instanceof Error ? err.message : "Couldn't revoke sessions"); } finally { setBusy(false); } }
  ...
  return (
    <div>
      ...
      <div className="mb-7 flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex gap-2"><Button variant="secondary" size="sm" onClick={() => window.open(`https://${storefrontDomain}`, "_blank")}>View storefront</Button><Button variant="secondary" size="sm">Impersonate admin</Button></div>
      </div>
